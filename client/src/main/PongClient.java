            ...
            while (!gameOver) {
                ...
                try {
                    ...
                    if (player.getPlayerNo() == PLAYER1) {
                        toServer.writeObject(new GameObjectPositions(ball.getX(), ball.getY(), ball.getxVel(), ball.getyVel(), player.getPaddle().getY(), player.getPaddle().getVelY()));
                        toServer.flush();
//                        sendPositions = new GameObjectPositions(ball.getX(), ball.getY(), ball.getxVel(), ball.getyVel(), player.getPaddle().getY(), player.getPaddle().getVelY());
                    } else {
                        toServer.writeObject(new GameObjectPositions(player.getPaddle().getY(), player.getPaddle().getVelY()));
                        toServer.flush();
